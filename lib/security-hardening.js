...
// In production, use PostgreSQL for sessions so multiple app instances can share
// authenticated state. Local development keeps the existing SQLite fallback.
if (process.env.DATABASE_URL) {
  try {
    ...
    const connectSqlitePath = require.resolve('connect-sqlite3');
    require.cache[connectSqlitePath].exports = () => PgStore;
  } catch (e) {
    console.error('[security-hardening] PostgreSQL session store unavailable:', e.message);
  }
}

// Keep downloads byte-correct: repair legacy marketplace .zip naming and resolve
// legacy /public/uploads paths without changing authorization checks in route code.
try {
  ...
  express.response.download = function(filePath, filename, options, callback) {
    ...
    if (!path.isAbsolute(resolvedPath)) resolvedPath = path.resolve(resolvedPath);
    if (!require('fs').existsSync(resolvedPath)) {
      const legacyPrefix = path.join(process.cwd(), 'public', 'uploads') + path.sep;
      if (resolvedPath.startsWith(legacyPrefix)) {
        resolvedPath = path.join(process.cwd(), 'uploads', path.basename(resolvedPath));
      }
    }
    let safeName = filename;
    if (safeName && /\.zip$/i.test(safeName)) {
      const ext = path.extname(resolvedPath) || '.bin';
      safeName = safeName.replace(/\.zip$/i, ext);
    }
    return originalDownload.call(this, resolvedPath, safeName, options, callback);
  };
} catch (_) {}

// Only image assets remain publicly reachable under /uploads. Course files,
// CVs, assignments and other documents must use authorization-aware endpoints.
