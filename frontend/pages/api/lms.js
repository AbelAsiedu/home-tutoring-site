...
async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (action === 'tutors') {
      if (!['admin','tutor','parent','student'].includes(role)) return forbidden(res)
  ...
  const handle=async(body,file)=>{
    ...
    if(action==='create_ward'){
      if(role!=='parent')return forbidden(res)
      const name=String(body.name||'').trim(); if(!name)return res.status(400).json({error:'Ward name is required'})
    ...
    if(action==='assign_tutor'){
      ...
      const tutor=await db.runQueryOne(`SELECT id FROM users WHERE id=? AND role='tutor'`,[body.tutor_id]); if(!tutor)return res.status(400).json({error:'Tutor not found'})
    ...
    if(action==='delete_ward'){
      if(role!=='admin')return forbidden(res)
      const files=await db.runQuery(`SELECT file_path FROM assignment_submissions WHERE ward_id=? AND file_path IS NOT NULL UNION ALL SELECT a.file_path FROM assignments a JOIN enrollments e ON e.id=a.enrollment_id WHERE e.ward_id=? AND a.file_path IS NOT NULL`,[body.ward_id,body.ward_id])
      for(const f of files)await deleteStoredFile(f.file_path)
    ...
    if(action==='create_assignment'){
      ...
      const enrollment=await db.runQueryOne(`SELECT e.*,w.name ward_name FROM enrollments e JOIN wards w ON w.id=e.ward_id WHERE e.id=?`,[body.enrollment_id]); if(!enrollment)return res.status(404).json({error:'Enrollment not found'})
      ...
      const id=uuidv4(),now=new Date().toISOString(),filePath=await persistUpload(file,'assignments')
      const ownerTutorId=role==='admin' ? (enrollment.tutor_id || user.id) : user.id
      await db.runExec(`INSERT INTO assignments (id,enrollment_id,tutor_id,title,instructions,due_date,file_path,total_points,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,[id,body.enrollment_id,ownerTutorId,title,body.instructions||null,body.due_date||null,filePath,Number(body.total_points)||100,body.status||'published',now,now])
    ...
    if(action==='grade_submission'){
      if(!['tutor','admin'].includes(role))return forbidden(res)
      const sub=await db.runQueryOne(`SELECT s.*,a.tutor_id FROM assignment_submissions s JOIN assignments a ON a.id=s.assignment_id WHERE s.id=?`,[body.submission_id]); if(!sub)return res.status(404).json({error:'Submission not found'})
      if(role==='tutor'&&sub.tutor_id!==user.id)return forbidden(res)
      const score=Number(body.score); if(!Number.isFinite(score)||score<0)return res.status(400).json({error:'Invalid score'})
      await db.runExec(`UPDATE assignment_submissions SET score=?,feedback=?,graded_at=?,status=? WHERE id=?`,[score,body.feedback||null,new Date().toISOString(),'graded',body.submission_id])
      return finish({success:true})
    }
    if(action==='delete_assignment'){
      if(!['tutor','admin'].includes(role))return forbidden(res)
      const a=await db.runQueryOne(`SELECT * FROM assignments WHERE id=?`,[body.assignment_id]); if(!a)return res.status(404).json({error:'Assignment not found'})
      if(role==='tutor'&&a.tutor_id!==user.id)return forbidden(res)
      const subs=await db.runQuery(`SELECT file_path FROM assignment_submissions WHERE assignment_id=? AND file_path IS NOT NULL`,[a.id]); for(const s of subs)await deleteStoredFile(s.file_path)
      await deleteStoredFile(a.file_path)
      await db.runExec(`DELETE FROM assignment_submissions WHERE assignment_id=?`,[a.id]); await db.runExec(`DELETE FROM assignments WHERE id=?`,[a.id])
      return finish({success:true})
    }
    return res.status(400).json({error:'unknown_action'})
  }
